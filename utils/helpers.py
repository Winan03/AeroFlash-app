...
def validate_flight_data(data):
    """Valida los datos requeridos del vuelo y devuelve un dict con el estado"""
    required_fields = ['origen', 'destino', 'fecha', 'hora_partida', 'numero_vuelo']
    for field in required_fields:
        if not data.get(field):
            return {'valid': False, 'message': f"Campo requerido faltante: {field}"}
    return {'valid': True}
